...
public class Screen extends JPanel implements KeyListener {
    ...
    public Screen() {
        ...
        try {
            LevelTwoBackground = resize(
                    ...
                    600);
            LevelOneBackground = resize(
                    ImageIO.read(getClass().getClassLoader().getResourceAsStream("Graphics/level1background.png")), 800,
                    600);
            LevelThreeBackground = resize(
                    ImageIO.read(getClass().getClassLoader().getResourceAsStream("Graphics/level3background.png")), 800,
                    600);
        } catch (IOException e) {
        }
                    ...
                    if (lives > 0)// as long as they have lives left, keep them alive
                    {
                        ...
                        if (level == 1) {
                            for (int i = 0; i < enemies.length; i++) {
                                ...
                                enemies[i].setSpeed(10);// move the enemies faster and make the level harder
                                enemies[i].scoreCount = false;// allow the enemies to be killed and added to the score
                                total = 0;// reset the total count so the player doesn't move onto the next level
                                          // without killing all 5 enemies
                            }
                        ...
                        score = 0;// reset score
                        lives = 4;// reset lives --> set to 4 because there is a bug if the user dies the lives
                                  // becomes -1 (using 4 to compensate)
                    }
                }
            } else {
                BE.move(f1);
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (code == 39)
            f1.moveRight();
        if (code == 32) {
            p1.setPosition(f1.getX() + 23, f1.getY());// so it looks like the main player is shooting from his gun
    ...
    private static BufferedImage resize(BufferedImage img, int newW, int newH) { // function from stack overflow that
                                                                                 // resizes the image
